...
test.pred <- predict(fit, test.data$x)-1

fitSGL <- SGL(train.data, membership.index, verbose = TRUE, nlam = 100, min.frac = 0.1, alpha = 0.05)

test.pred.SGL <- matrix(NA, ncol = length(fitSGL$lambdas), nrow = length(test.data$y))

for(i in 1:length(fitSGL$lambdas)){
  test.pred.SGL[,i] <- predict(x=fitSGL,newX=test.data$x,y=test.data$y,lam=i)
}
...
ggplot(data = dd, aes(x = x, y = y, group = Method, shape = Method)) + geom_line(aes(linetype=Method), size = 1.5) + scale_y_continuous("Correct Classification Rate") + scale_x_continuous("Lambda Index")  + scale_linetype_manual(values=c("twodash","dotted","solid"))
